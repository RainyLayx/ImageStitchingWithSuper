...
if __name__ == '__main__':
    ...
    # parser.add_argument(
    #     '--resize', type=int, nargs='+', default=[480, 640],
    #     help='Resize the input image before running inference. If two numbers, '
    #          'resize to the exact dimensions, if one number, resize the max '
    #          'dimension, if -1, do not resize')

    parser.add_argument(
        '--max_length', type=int, nargs='+', default=1280,
        help='Resize the input image before running inference. If two numbers, '
             'resize to the exact dimensions, if one number, resize the max '
             'dimension, if -1, do not resize')
    parser.add_argument(
    ...
    opt.input_dir = r'./testdir/test'
    opt.output_dir = opt.input_dir
    opt.input_pairs = r'./testdir/test.txt'
    print(opt)
    ...
    for i, pair in enumerate(pairs):
        ...
        # Load the image pair.
        image0,_, inp0, scales0 = read_image(
            os.path.join(input_dir,name0), device, opt.max_length, rot0, opt.resize_float)
        image1,_, inp1, scales1 = read_image(
            os.path.join(input_dir,name1), device, opt.max_length, rot1, opt.resize_float)
        ...
        if do_match:
            ...
            # pdb.set_trace()
            warpImg = cv2.warpPerspective(image1, Mat, (image0.shape[1],int(image0.shape[0]+image1.shape[0])))
            direct=warpImg.copy()
            direct[0:image0.shape[0], 0:image0.shape[1]] = image0

            rows,cols=image0.shape[:2]
            # drawMatches(image0, image1, PtsA, PtsB, Match, status)
            for row in range(0,rows):
                if image0[row, :].any() and warpImg[row, :].any():#开始重叠的最左端
                    top = row
                    break
            for row in range(rows-1, 0, -1):
                if image0[row, :].any() and warpImg[row, :].any():#重叠的最右一列
                    bot = row
                    break
            ...
            for col in range(0, cols):
                for row in range(0, rows):
                    if not image0[row, col].any():#如果没有原图，用旋转的填充
                        res[row, col] = warpImg[row, col]
                    elif not warpImg[row, col].any():
                        res[row, col] = image0[row, col]
                    else:
                        srcImgLen = float(abs(row - top))
                        testImgLen = float(abs(row - bot))
                        alpha = srcImgLen / (srcImgLen + testImgLen)
                        res[row, col] = np.clip(image0[row, col] * (1-alpha) + warpImg[row, col] * alpha, 0, 255)

            warpImg[0:image0.shape[0], 0:image0.shape[1]]=res
            cv2.imwrite(str(matches_path),warpImg)
